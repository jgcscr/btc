...
- **CryptoQuant daily fallback**: Daily metrics are ingested via `data/ingestors/cryptoquant_daily.py` while hourly access remains blocked (ticket `CQ-2025-1213`). The resampled hourly features live at `data/processed/cryptoquant/hourly_features.parquet`, with coverage details logged in `artifacts/monitoring/cryptoquant_daily_summary.json`.
