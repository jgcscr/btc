...
- Monthly walk-forward refresh (train 1m / val 1m / test 1m) using the volatility-aware direction dataset and the current ensemble stack delivered:
  - **Ensemble (p_up ≥ 0.60, ret_pred ≥ 5e-4)**: 233 trades, hit rate 96.6%, avg ret/trade 39 bps, net cumulative log return +0.91, max drawdown 2.6%.
  - **Direction-only baseline (p_up ≥ 0.50)**: 280 trades, hit rate 95.0%, avg ret/trade 34 bps, net cumulative log return +0.96, max drawdown 3.5%.
  - Outputs live under `artifacts/walkforward/monthly_cpu/test_20251201_1m/` with the high-level summary mirrored to `artifacts/walkforward/monthly_cpu/summary_latest.json`.

### Phase 1 Status – 2025-12-13

- Ready feeds: macro (SPY 1h, realized vol), Binance spot/perp metrics (funding + realized vol via `compute_funding_features.py`), daily CryptoQuant fallback (forward-filled to hourly).
- Fallbacks: CryptoQuant hourly pending CQ-2025-1213; Binance REST occasionally rate-limits long historical pulls but the local parquet cache keeps coverage stable.
...
- Next activation step once vendors unblock: rerun `cryptoquant_daily.py` → `compute_cryptoquant_resampled.py` → rebuild `build_training_dataset*.py` to refresh NPZ artifacts.
...
- **In-house derivatives online**: `data/processed/compute_macro_features.py` and `data/processed/compute_funding_features.py` now compute realized volatility (1h absolute log-return, 24h rolling std), basis spreads (perp − spot with 24h mean), and funding-rate deltas (1h, 24h). Outputs are stored alongside existing features and monitored via the refreshed summaries in `artifacts/monitoring/macro_summary.json` and `artifacts/monitoring/funding_summary.json`.
- **Remaining data gaps**: True hourly CryptoQuant series still requires the vendor fix; downstream datasets rely on the daily fallback plus existing macro/on-chain feeds until that ticket clears.
