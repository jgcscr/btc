...
def _validate_conv_params(
    ...
    if len(channels) > 2:
        raise ValueError("cnn-lstm classifier supports at most two conv layers")
...
class CNNLSTMDirectionClassifier(nn.Module):
    def __init__(
        self,
        *,
        input_size: int,
        ...
        self.norm = _build_norm(norm_type, lstm_hidden_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # type: ignore[override]
        ...
        # Conv1d expects [batch, channels, seq_len]; treat feature dimension as channels.
...
__all__ = ["CNNLSTMDirectionClassifier"]
