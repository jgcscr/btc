...
from src.scripts.train_cnn_lstm_dir1h import _parse_int_list
...
def _resolve_conv_params(args: argparse.Namespace) -> tuple[List[int], List[int], List[int]]:
    ...
    if len(channels) > 2:
        raise ValueError("CNN-BiLSTM trainer supports at most two convolutional layers")
    return channels, kernels, strides


def _build_cnn_bilstm_classifier(input_size: int, args: argparse.Namespace) -> nn.Module:
    return CNNBiLSTMDirectionClassifier(
...
def parse_args() -> argparse.Namespace:
    parser = build_arg_parser(
        description="Train a CNN-BiLSTM hybrid classifier for 1h BTC direction.",
        default_output_dir="artifacts/models/cnn_bilstm_dir1h_v1",
    )
    parser.add_argument(
        "--conv-channels",
        type=str,
...
def train_cnn_bilstm(args: argparse.Namespace) -> None:
    ...
    print("Resolved CNN-BiLSTM hyperparameters:")
    print(json.dumps(resolved, indent=2))

    train_model(
        args,
        model_builder=_build_cnn_bilstm_classifier,
        model_label="cnn_bilstm_direction_classifier",
